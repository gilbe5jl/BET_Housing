...
class create_cycle:
    ...
    def cycle(self ):
        ...
        with LogixDriver(config_info['plc_ip']) as plc: #context manager for plc connection, currently resetting connection after ~24 hrs to avoid issues
            ...
            print_color(f'({machine_num}) PLC & Keyence Connection Successful...\n')
           # setKeyenceRunMode(machine_num, sock)
            ...
            while(True):
                ...
                if(self.current_stage == 0):
                    ...
                    if keyence_string == 'ERROR': kill_threads.set()                               
                ...
                #################### STAGE ONE ####################
                    ...
                    print_blue(f'({machine_num})[STAGE:1] Waiting for START_PROGRAM...\n')
