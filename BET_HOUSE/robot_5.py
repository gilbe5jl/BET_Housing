...
def read_config()->dict:
    with open(os.path.join(sys.path[0], 'config_r5.json'), "r") as config_file:
           ...
           # setKeyenceRunMode(machine_num, sock)
            ...
            while(True):
                ...
                if(self.current_stage == 0):
                    ...
                    if (kill_threads.is_set()): #check for reset at beginning of cycle
                        break
                    print_color(f'({machine_num}) PLC(LOAD_PROGRAM) is HIGH gathering PLC(PART_PROGRAM) & PLC(PART_TYPE)...') # Once PLC(LOAD_PROGRAM) goes high, mirror data and set Phoenix(READY) high, signifies end of "loading" process
                ...
                #################### STAGE ONE ####################
                    ...
                    print_color(f'({machine_num})[STAGE:1] PLC(START_PROGRAM) activated at {datetime.datetime.now()}\n')
                    ...
                    print_color(f'({machine_num})[STAGE:1] Completed at [{datetime.datetime.now()}]...\n')
